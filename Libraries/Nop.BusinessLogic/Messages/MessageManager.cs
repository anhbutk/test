...
namespace NopSolutions.NopCommerce.BusinessLogic.Messages
{
    ...
    public partial class MessageManager
    {
        ...
        private static string ProductListToHtmlTable(Order order, int LanguageID)
        {
            ...
            sb.AppendLine("<table class=\"table\" border=\"1\" style=\"border:1px solid grey;padding:2px;border-collapse:collapse;width:100%\"><thead><tr>");

            sb.AppendLine("<th class=\"header\" style=\"text-align:center;background-color:#EFE6D5\">" + LocalizationManager.GetLocaleResourceString("Order.ProductsGrid.Name", LanguageID) + "</th>");
            sb.AppendLine("<th class=\"header\" style=\"text-align:center;background-color:#EFE6D5\">" + LocalizationManager.GetLocaleResourceString("Order.ProductsGrid.Price", LanguageID) + "</th>");
            sb.AppendLine("<th class=\"header\" style=\"text-align:center;background-color:#EFE6D5\">" + LocalizationManager.GetLocaleResourceString("Order.ProductsGrid.Quantity", LanguageID) + "</th>");
            sb.AppendLine("<th class=\"header\" style=\"text-align:center;background-color:#EFE6D5\">" + LocalizationManager.GetLocaleResourceString("Order.ProductsGrid.Total", LanguageID) + "</th>");
            ...
            for (int i = 0; i <= table.Count - 1; i++)
            {
                ...
                sb.AppendLine("<td class=\"row\" style=\"text-align:right\">" + unitPriceStr + "</td>");

                sb.AppendLine("<td class=\"row\" style=\"text-align:center\">" + table[i].Quantity + "</td>");
                ...
                sb.AppendLine("<td class=\"row\" style=\"text-align:right\">" + priceStr + "</td>");
            ...
            sb.AppendLine("<tr><td style=\"text-align:right;\" colspan=\"3\"></td><td style=\"text-align:right;\">");
            sb.AppendLine("<table class=\"table\" style=\"border:0px solid grey;padding:2px;border-collapse:collapse;float:right\">");
        ...
        /// <param name="LanguageID">Message language identifier</param>
        /// <returns>Queued email identifier</returns>
        public static int SendCustomerEmailValidationMessage(Customer customer, int LanguageID, string Password)
        {
            if (customer == null)
                throw new ArgumentNullException("customer");
        ...
        public static int SendEmailAFriendMessageAll(Customer customer, int LanguageID, string FriendsEmail, string FriendsName, string YourEmail, string YourName, string PersonalMessage, string URL)
        {
            //if (customer == null)
            //    throw new ArgumentNullException("customer");

            string TemplateName = "Service.EmailAFriendAll";
            LocalizedMessageTemplate localizedMessageTemplate = MessageManager.GetLocalizedMessageTemplate(TemplateName, LanguageID);
            if (localizedMessageTemplate == null)
                return 0;
            //throw new NopException(string.Format("Message template ({0}-{1}) couldn't be loaded", TemplateName, LanguageID));
            ...
            NameValueCollection additinalKeys = new NameValueCollection();
            additinalKeys.Add("EmailAFriend.PersonalMessage", PersonalMessage);
            additinalKeys.Add("Customer.Email", FriendsName);
            additinalKeys.Add("Customer.FullName", YourName);
            additinalKeys.Add("Product.ProductURLForCustomer", URL);
            string subject = ReplaceMessageTemplateTokens(localizedMessageTemplate.Subject, additinalKeys);
            string body = ReplaceMessageTemplateTokens(localizedMessageTemplate.Body, additinalKeys);
            MailAddress from = new MailAddress(AdminEmailAddress, AdminEmailDisplayName);
            MailAddress to = new MailAddress(FriendsEmail);
            QueuedEmail queuedEmail = InsertQueuedEmail(5, from, to, string.Empty, string.Empty, subject, body, DateTime.Now, 0, null);
            return queuedEmail.QueuedEmailID;
        ...
        public static int SendCustomerWinnerMessage(int LanguageID, string FullName, DateTime CompleteDate, Customer customer)
        {
            string TemplateName = "Customer.CongratWinner";
            LocalizedMessageTemplate localizedMessageTemplate = MessageManager.GetLocalizedMessageTemplate(TemplateName, LanguageID);
            if (localizedMessageTemplate == null)
                return 0;
            //throw new NopException(string.Format("Message template ({0}-{1}) couldn't be loaded", TemplateName, LanguageID));
            ...
            tokens.Add("Month", CompleteDate.Month.ToString());
            tokens.Add("Year", CompleteDate.Year.ToString());

            foreach (string token in tokens.Keys)
            {
                subject = subject.Replace(string.Format(@"%{0}%", token), tokens[token]);
                body = body.Replace(string.Format(@"%{0}%", token), tokens[token]);
            }
        ...
        public static int SendCustomerCompletePlayer(int LanguageID, string FullName, DateTime CompleteDate, string[] result, Customer customer)
        {
            string TemplateName = "Customer.CompletePlayer";
            LocalizedMessageTemplate localizedMessageTemplate = MessageManager.GetLocalizedMessageTemplate(TemplateName, LanguageID);
            if (localizedMessageTemplate == null)
                return 0;
            //throw new NopException(string.Format("Message template ({0}-{1}) couldn't be loaded", TemplateName, LanguageID));
            ...
            tokens.Add("Month", CompleteDate.Month.ToString());
            tokens.Add("Year", CompleteDate.Year.ToString());
            tokens.Add("Result", AnswerToTable(result, LanguageID));
            tokens.Add("CompleteDate", CompleteDate.ToString("dd/MM/yyyy hh:mm"));
        ...
        private static string AnswerToTable(string[] resultArr, int LanguageID)
        {
            string result = string.Empty;
            StringBuilder sb = new StringBuilder();
            sb.AppendLine("<table class=\"table\" border=\"1\" style=\"border:1px solid grey;padding:2px;border-collapse:collapse;width:100%\"><thead><tr>");

            sb.AppendLine("<th class=\"header\" style=\"text-align:center;background-color:#EFE6D5\">" + LocalizationManager.GetLocaleResourceString("Game1Play.Question", LanguageID) + "</th>");
            sb.AppendLine("<th class=\"header\" style=\"text-align:center;background-color:#EFE6D5\">" + LocalizationManager.GetLocaleResourceString("Game1Play.YourAnswer", LanguageID) + "</th>");
            sb.AppendLine("<th class=\"header\" style=\"text-align:center;background-color:#EFE6D5\">" + LocalizationManager.GetLocaleResourceString("Game1Play.CorrectAnswer", LanguageID) + "</th>");

            sb.AppendLine("</tr></thead><tbody>");

            for (int i = 0; i < resultArr.Length; i++)
            {
                string[] items = resultArr[i].Split(';');
                sb.AppendLine("<tr>");

                sb.AppendLine("<td class=\"row\">" + items[0] + "</td>");

                sb.AppendLine("<td class=\"row\">" + items[1] + "</td>");

                sb.AppendLine("<td class=\"row\">" + items[2] + "</td>");
        ...
        public static string ReplaceMessageTemplateTokens(Order order, string Template, int LanguageID)
        {
            ...
            tokens.Add("Order.CustomerNote", HttpUtility.HtmlEncode(order.CustomerNote));
            tokens.Add("Order.PaymentMethod", HttpUtility.HtmlEncode(order.PaymentMethod.VisibleName));
            tokens.Add("Order.Product(s)", ProductListToHtmlTable(order, LanguageID));
            tokens.Add("Order.SavePoint", order.OrderTotal.ToString("#").Replace("000", ""));
            tokens.Add("Order.TotalSavePoint", order.CustomerSavePoint);
        ...
        public static string ReplaceMessageTemplateTokens(string Template, NameValueCollection AdditinalKeys)
        {
            NameValueCollection tokens = new NameValueCollection();
            tokens.Add("Store.Name", SettingManager.StoreName);
            tokens.Add("Store.URL", SettingManager.StoreURL);
