...
namespace NopSolutions.NopCommerce.BusinessLogic.Utils
{
    ...
    public partial class PDFHelper
    {
        ...
        public static void PrintOrderToPDF(Order order, int LanguageID, string FilePath, string FontPath)
        {
            if (order == null)
            ...
            doc.Add(new Paragraph("   " + String.Format(LocalizationManager.GetLocaleResourceString("PDFInvoice.Name", LanguageID), order.BillingFullName), f));
            ...
            doc.Add(new Paragraph("   " + String.Format(LocalizationManager.GetLocaleResourceString("PDFInvoice.Address", LanguageID), order.BillingAddress1), f));
            ...
            if (order.ShippingStatus != ShippingStatusEnum.ShippingNotRequired)
            {
                ...
                doc.Add(new Paragraph("   " + String.Format(LocalizationManager.GetLocaleResourceString("PDFInvoice.Name", LanguageID), order.ShippingFullName), f));
                ...
                doc.Add(new Paragraph("   " + String.Format(LocalizationManager.GetLocaleResourceString("PDFInvoice.Address", LanguageID), order.ShippingAddress1), f));
            ...
            for (int i = 0; i < productCollection.Count; i++)
            {
                ...
                tbl.AddCell(new Cell(new Paragraph(name, f)), row, 0);
