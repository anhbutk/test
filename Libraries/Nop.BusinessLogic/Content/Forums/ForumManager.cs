...
namespace NopSolutions.NopCommerce.BusinessLogic.Content.Forums
{
    ...
    public partial class ForumManager
    {
        ...
        private static ForumTopic DBMapping(DBForumTopic dbItem)
        {
            ...
            item.CreatedOn = dbItem.CreatedOn;
            item.UpdatedOn = dbItem.UpdatedOn;
            item.TopicContent = dbItem.TopicContent;
            item.VideoClip = dbItem.VideoClip;
            item.HomePage = dbItem.HomePage;
            item.OnOff = dbItem.OnOff;
            item.PictureID = dbItem.PictureID;
            item.ShortContent = dbItem.ShortContent;
        ...
        /// 
        /// </summary>
        /// <param name="ForumGroupID"></param>
        /// <returns></returns>
        public static ForumCollection GetAllForums()
        {

            DBForumCollection dbCollection = DBProviderManager<DBForumProvider>.Provider.GetAllForums();
            ForumCollection forumCollection = DBMapping(dbCollection);
        ...
        public static ForumTopicCollection GetTopicsByForumID(int ForumID)
        {
            DBForumTopicCollection dbCollection = DBProviderManager<DBForumProvider>.Provider.GetTopicsByForumID(ForumID);
            ForumTopicCollection forumTopicCollection = DBMapping(dbCollection);

            return forumTopicCollection;
        }

        public static ForumTopicCollection GetOlderTopicsByTopicIDForumID(int ForumID, DateTime TopicDate)
        {
            DBForumTopicCollection dbCollection = DBProviderManager<DBForumProvider>.Provider.GetOlderTopicsByTopicIDForumID(ForumID, TopicDate);
            ForumTopicCollection forumTopicCollection = DBMapping(dbCollection);

            return forumTopicCollection;
        }

        public static ForumTopicCollection GetAllTopicsExtreme()
        {
            DBForumTopicCollection dbCollection = DBProviderManager<DBForumProvider>.Provider.GetAllTopicsExtreme();
            ForumTopicCollection forumTopicCollection = DBMapping(dbCollection);
        ...
        public static ForumTopic InsertTopic(int ForumID, int UserID,
            ...
            DateTime CreatedOn, DateTime UpdatedOn, bool SendNotifications, string TopicContent,
            string VideoClip, bool Homepage, bool OnOff, int PictureID, string ShortContent)
        {
            CreatedOn = DateTimeHelper.ConvertToUtcTime(CreatedOn);
            UpdatedOn = DateTimeHelper.ConvertToUtcTime(UpdatedOn);
            ...
            DBForumTopic dbItem = DBProviderManager<DBForumProvider>.Provider.InsertTopic(ForumID, UserID,
                ...
                LastPostUserID, LastPostTime, CreatedOn, UpdatedOn, TopicContent, VideoClip,
                Homepage, OnOff, PictureID, ShortContent);
            ...
            bool Homepage, bool OnOff, int PictureID, string ShortContent, int Position)
        {
            ...
            DBForumTopic dbItem = DBProviderManager<DBForumProvider>.Provider.UpdateTopic(ForumTopicID, ForumID, UserID,
                (int)TopicType, Subject, NumPosts, Views, LastPostID,
                LastPostUserID, LastPostTime, CreatedOn, UpdatedOn, TopicContent, VideoClip,
                Homepage, OnOff, PictureID, ShortContent, Position);
                ...
                if (newForum != null)
                {
                    if (previousForumID != NewForumID)
                    {
                        forumTopic = UpdateTopic(forumTopic.ForumTopicID, newForum.ForumID,
                            ...
                            forumTopic.TopicContent, forumTopic.VideoClip, forumTopic.HomePage,
                            forumTopic.OnOff, forumTopic.PictureID, forumTopic.ShortContent, forumTopic.Position);
