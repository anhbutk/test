...
namespace NopSolutions.NopCommerce.BusinessLogic.Orders
{
    ...
    public partial class OrderManager
    {
        ...
        private static Order DBMapping(DBOrder dbItem)
        {
            ...
            item.CustomerNote = dbItem.CustomerNote;
        ...
        protected static Order SetOrderStatus(int OrderID, OrderStatusEnum OS, bool notifyCustomer)
        {
            ...
            if (order != null)
            {
                ...
                order = UpdateOrder(order.OrderID, order.OrderGUID, order.CustomerID, order.CustomerLanguageID,
                    ...
                    order.Deleted, order.CreatedOn, order.CustomerNote);
            }
            return order;
        }
            ...
            if (order != null)
            {
                UpdateOrder(order.OrderID, order.OrderGUID, order.CustomerID, order.CustomerLanguageID,
                   ...
                   order.CardName, order.CardNumber, order.MaskedCreditCardNumber,
                    ...
                    order.CreatedOn, order.CustomerNote);
            }
        }
        ...
        public static Order InsertOrder(Guid OrderGUID, int CustomerID, int CustomerLanguageID,
            ...
            bool Deleted, DateTime CreatedOn, string CustomerNote)
        {
            ...
            DBOrder dbItem = DBProviderManager<DBOrderProvider>.Provider.InsertOrder(OrderGUID, CustomerID, CustomerLanguageID,
             ...
             Deleted, CreatedOn, CustomerNote);
        ...
        public static string PlaceOrder(PaymentInfo paymentInfo, Customer customer, out int OrderID, string CustomerNote)
        {
            Guid OrderGuid = Guid.NewGuid();
            return PlaceOrder(paymentInfo, customer, OrderGuid, out OrderID, CustomerNote);
        }
        ...
        public static string PlaceOrder(PaymentInfo paymentInfo, Customer customer, Guid OrderGuid,
            out int OrderID, string customerNote)
        {
            OrderID = 0;
        ...
        ///<returns></returns>
        public static string GetCustomerSavePoint(int CustomerID)
        {
            decimal savepoint = 0;
            OrderCollection list = GetOrdersByCustomerID(CustomerID);
            List<Order> listOrder = list.FindAll(IsCompleted).FindAll(IsInYear);
            foreach (var item in listOrder)
            {
                savepoint += item.OrderTotal;
            }
            return PriceHelper.SavePoint(savepoint);
        }

        static bool IsCompleted(Order x)
        {
            return x.PaymentStatusID == 30;
        }

        static bool IsInYear(Order x)
        {
            return x.CreatedOn.Year >= int.Parse(SettingManager.GetSettingValue("Customer.SavePointYear"));
        }
        #endregion
